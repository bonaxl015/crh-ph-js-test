...
const $ = {
  ...
  map: function (number, func) {
    return number.map(func);
  },
  reduce: function (number, func, initial = null) {
    return (number.length === 0 && initial === null ? number : number.reduce(func, initial));
  },
  head: function (arr) {
    return arr[0];
  },
